...
- [ ] **Phase 3C (remainder) — rendering performance.** The cheap wins are done (search debounced, tap targets raised, tabs wrap, card indent reduced). Still outstanding: `render()` rebuilds **every** matching card into one `innerHTML` — ~35-50k DOM nodes for 1273 articles, with all abstracts already in the DOM. Real fix is rendering only expanded tiers or windowing. Measure on an actual phone before and after.
